...
- **智能出题模式** — 非分批模式下自动按题目数量随机采样等量知识点 + 关联知识点作为 context，避免堆入无关内容；估算输出 token 超出上限时自动切换分批（无需手动开启）；分批模式支持顺序切片（KP数 ≤ 题数×3）和**稀疏随机采样**（KP数 > 题数×3）两种策略，各批并发调用 DeepSeek
...
- **题库筛选入池** — 按科目/题型/难度/语言/知识点/标签/**题干关键词**多维筛选；预览命中后可通过**多选框逐题勾选**（全选/全不选），仅将勾选题目加入池；自动排除已在池中的题目
...
- **套题质量检查（SSE 实时进度）** — 历史场次每行新增「检查」按钮，调用 DeepSeek 对场次全部题目进行质量扫描：① 题干答案混淆；② 英文题干语法错误；检查过程通过 SSE 流式推送实时进度条（显示"正在检查第 X / N 题"及百分比）；结果展示原文/建议对比，可勾选后一键批量修复；关闭弹窗自动断开 SSE
- **历史场次删除** — 历史场次列表每行新增「删除场次」按钮，删除后级联清除所有套题并自动重置受影响题目的 `drawn` 状态
- **套题替换（含候选搜索）** — 查看套题时每道题旁「替换」按钮内置搜索框，可按**题干关键词**或**知识点**实时过滤候选题（客户端过滤，上限 300 道），知识点以蓝色标签展示
- **套题在线编辑** — 查看套题时每道题旁「编辑」按钮可在线修改题干/答案，替换后自动维护 `drawn` 状态
- **随机抽选全屏展示** — 点击「随机抽取」后弹出覆盖全屏的优雅展示页面（深色主题、大字体 1.25rem、无答案、A/B/C… 彩色标识），专为面试者设计；抽取套数可在输入框自由设置（默认 1，最多 20）；顶部固定「关闭展示」按钮
- **一键释放已使用套题** — 场次卡片在有已使用套题时显示绿色「一键释放 (N)」按钮，一次性重置该场次全部已使用套题及关联题目 `drawn` 状态，方便测试后快速复位
- **套题状态管理** — 逐套/批量标记已使用，释放时重置题目在池中的可抽状态
- **Word 导出** — 套题导出为富文本 Word（图片/表格保留），可选含答案，每套分页
- **Excel 跨机器迁移导入** — 导入套题时自定义场次名；无题库池则自动新建，已有池则弹框选择加入方式；加入已有池时自动跳过重复题目；导入完成自动跳转套题管理并显示摘要；题库池单独导出 Excel 同样支持
...
│   ├── interview_routes.py         # 面试抽题 API（35 个端点）
...
| `POST` | `/api/interview/sessions/<id>/quality-check` | DeepSeek 检查场次题目质量（题干答案混淆 + 英文语法）**v1.26** |
