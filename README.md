...
- **分批并行出题** — 分批模式下系统自动判断：若知识点总数 ≤ 题目数×3，按顺序切片分批，每批并发调用 DeepSeek；若知识点总数 > 题目数×3（全选文档时常见），切换为**稀疏随机采样模式**：每批从全部知识点中随机采样该批题目数量的知识点，并自动扩展其关联知识点作为 context，各批独立并发执行，避免大型知识库时上下文截断或题目同质化
