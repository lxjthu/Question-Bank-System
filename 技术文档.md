...
```sql
-- 文档元信息
CREATE TABLE ds_docs (
    doc_id     TEXT PRIMARY KEY,   -- 文件名去扩展名，截断至 40 字符
    filename   TEXT,               -- 原始文件名
    subject    TEXT,               -- 学科提示（传给 DeepSeek 提取 prompt）
    status     TEXT DEFAULT 'uploaded',  -- uploaded / extracting / done / error
    error_msg  TEXT,
    created_at TEXT
);

-- 章节原文（解析时拆分）
CREATE TABLE ds_chapters (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
    doc_id       TEXT,
    chapter_num  INTEGER,          -- 从 0 开始，按文本顺序
    chapter_name TEXT,             -- 标题行内容
    raw_text     TEXT              -- 该章节所有正文，最长截断至 8000 字符再传 API
);
...
CREATE TABLE ds_kps (
    ...
    chapter_name   TEXT,
    chapter_num    INTEGER,
    kp_name        TEXT,           -- 5-20 字的知识点名称
    kp_content     TEXT,           -- 200-500 字详细说明（尽量引用原文）
    relations_json TEXT DEFAULT '[]'  -- JSON 数组：[{"type": "从属", "target": "..."}]
);
```
...
DS 模式上传 PDF/PPTX 时，`_ocr_then_parse` 内联工作函数完全复用 `pptx_ocr.pipeline` 的已有逻辑：
...
【学科/科目】：{subject}
【章节名称】：{chapter_name}
【章节原文】：{chapter_text（≤8000 字）}

要求：
- 提取 3-8 个核心知识点
- content 字段引用原文，200-500 字，含定义/特征/分类/示例
- relations 仅标注同章节内知识点关系
- 输出纯 JSON，不含代码块标记
```

DeepSeek 温度设为 `0.3`（提取阶段，追求准确），出题阶段 `0.7`（追求多样性）。
