...
| 字段 | 类型 | 说明 |
|------|------|------|
| `subject` | VARCHAR(128) | 组卷时选择的科目筛选值；自动组卷时存入 |
| `is_confirmed` | BOOLEAN | 是否已最终确认；默认 False；确认后题目锁定 |
| `confirmed_at` | DATETIME | 最终确认时间戳；取消确认后清空为 NULL |
...
将试卷中所有题目标记为 `is_used=True`，同时设置 `exam.is_confirmed=True`、`exam.confirmed_at=now`。确认后，`add_question`、`remove_question`、`replace_question` 接口将返回 `403`，禁止修改。

#### `POST /api/exams/<id>/revert_confirmation` — 取消确认

将试卷中所有题目标记为 `is_used=False`、`used_date=None`，同时清空 `exam.is_confirmed=False`、`exam.confirmed_at=None`，解除锁定。
...
### 8.0 自动 Schema 迁移（`_migrate_db`，v1.7+）

`factory.py` 中的 `_migrate_db()` 在每次应用启动时执行，幂等地向已有 `exams` 表补充新字段：
...
`db.create_all()` 只会新建不存在的表，不会修改已有表结构，因此对于已部署实例需要此迁移函数。
...
| `POST` | `/api/rag/ds-extract/<doc_id>` | 启动异步任务；`resume=true` 跳过已提取章节实现断点续传（`temp=0.3`，每章最多 3000 tokens） |
| `GET` | `/api/rag/ds-tasks/<task_id>` | 轮询任务状态，返回 `{status, message, progress, total, total_chapters, kp_count}` |
| `POST` | `/api/rag/ds-tasks/<task_id>/pause` | 请求暂停正在运行的提取任务，设置 `pause_requested=True`，Worker 在当前章节完成后优雅退出 |
| `POST` | `/api/rag/ds-retry-ocr/<task_id>` | OCR 超时断点续传 |
| `GET` | `/api/rag/ds-docs` | 列出所有 DS 文档，含 `kp_count`、`chapter_count`、`ch_with_kps`、`status` |
...
`POST /api/rag/ds-extract/<doc_id>` 增加 `resume` 参数：

```
resume=false（默认）：DELETE FROM ds_kps WHERE doc_id=?，从头提取所有章节
resume=true          ：保留已有 KP，查询 DISTINCT chapter_num，过滤已提取章节后只处理剩余部分
                       若所有章节已完成，立即返回 {already_done: true}，不启动线程
```

**随时暂停（Pause）：**

任务状态字典（内存 `_ds_tasks`）新增 `pause_requested` 标志：

```python
# Worker 每章开始前检查
if _ds_tasks[task_id].get('pause_requested', False):
    conn.execute("UPDATE ds_docs SET status='paused' WHERE doc_id=?", ...)
    _ds_tasks[task_id] = {'status': 'paused', 'message': '已暂停...', ...}
    return   # 优雅退出，不影响已提取的 KP

# pause 端点仅设置标志，不强制终止线程
...
```
uploaded → extracting → done
                     ↘ paused → extracting（续传）→ done
                     ↘ error
```

**前端交互：**

- 提取进行中：进度条右侧显示橙色「暂停」按钮（`pauseDsExtract()`）
- 文档状态 `paused`：图标变为 ⏸，副标题显示「已提取 X/Y 章 · N 个知识点」，操作按钮为「继续」（`dsExtract(docId, true)`）和「重新提取 ↺」
- `paused` 状态的文档也可参与出题（`renderDsDocCheckboxes` 接受 `done || paused`）
