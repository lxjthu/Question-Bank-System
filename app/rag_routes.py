...
# DS 出题任务状态表  {task_id: {status, content, error, stats}}
_gen_tasks: dict = {}
...
def _get_user_api_key() -> str:
    """优先用当前用户的 API Key，无则 fallback 到 .env/环境变量。"""
    try:
...
def _check_doc_access(doc_id, user, conn):
    """验证用户对文档的访问权限。返回 (doc_row, None) 或 (None, (jsonify_resp, status_code))。
    admin 可访问所有文档；owner_id IS NULL 的旧文档所有人可访问。"""
    doc = conn.execute(
        "SELECT doc_id, owner_id FROM ds_docs WHERE doc_id=?", (doc_id,)
    ).fetchone()
    if doc is None:
        return None, (jsonify({'error': f'文档 {doc_id} 不存在'}), 404)
    if user.role != 'admin' and doc['owner_id'] is not None and doc['owner_id'] != user.id:
        return None, (jsonify({'error': '无权访问此文档'}), 403)
    return doc, None
...
def ds_delete_doc(doc_id):
    ...
    with _ds_db_conn() as conn:
        if user and getattr(user, 'role', None) != 'admin':
            doc = conn.execute(
                "SELECT owner_id FROM ds_docs WHERE doc_id=?", (doc_id,)
            ).fetchone()
            if doc and doc['owner_id'] is not None and doc['owner_id'] != user.id:
                return jsonify({'error': '无权限删除此文档'}), 403
...
def ds_upload():
    ...
    try:
        ...
        _upload_owner_id = _upload_user.id if _upload_user else None
        with _ds_db_conn() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO ds_docs "
                "(doc_id, filename, subject, status, error_msg, created_at, owner_id) "
                "VALUES (?,?,?,'uploaded','',?,?)",
                (doc_id, filename, subject, datetime.datetime.now().isoformat(), _upload_owner_id),
            )
            conn.execute("DELETE FROM ds_chapters WHERE doc_id=?", (doc_id,))
            conn.execute("DELETE FROM ds_kps WHERE doc_id=?", (doc_id,))
            for sec in sections:
...
def ds_extract(doc_id):
    ...
    with _ds_db_conn() as conn:
        _, err = _check_doc_access(doc_id, user, conn)
        if err:
            return err
    ...
    # 在请求上下文中获取 API Key，再传给后台线程（线程内无 request 上下文）
    extract_api_key = _get_user_api_key()
    ...
    def _worker(task_id, doc_id, subject, all_chapters, chapters,
                arch_json_str, concurrency, api_key):
        try:
            if not api_key:
                with _ds_db_conn() as conn:
                    conn.execute(
    ...
    t = threading.Thread(
        ...
        args=(task_id, doc_id, subject, list(all_chapters),
              list(chapters), arch_json_str, concurrency, extract_api_key),
        daemon=True,
    )
    t.start()
...
def ds_kp_get(kp_id):
    ...
    with _ds_db_conn() as conn:
        ...
        _, err = _check_doc_access(row['doc_id'], user, conn)
        if err:
            return err
    try:
...
def ds_kp_update(kp_id):
    ...
    with _ds_db_conn() as conn:
        ...
        if not old:
            return jsonify({'error': f'知识点 {kp_id} 不存在'}), 404
        _, err = _check_doc_access(old['doc_id'], user, conn)
        if err:
            return err
...
def ds_batch_classify():
    ...
    with _ds_db_conn() as conn:
        if kp_ids:
            ph = ','.join('?' * len(kp_ids))
            rows = conn.execute(
                f"SELECT id, doc_id, kp_name, kp_content, knowledge_type, cognitive_dimension "
            ...
            for r in rows:
                if r['doc_id'] not in seen_docs:
                    _, err = _check_doc_access(r['doc_id'], user, conn)
                    if err:
                        return err
            ...
            if err:
                return err
            rows = conn.execute(
                "SELECT id, doc_id, kp_name, kp_content, knowledge_type, cognitive_dimension "
    ...
    with _ds_db_conn() as conn:
        if not doc_ids:
            if user.role == 'admin':
                ...
                rows_d = conn.execute(
                    "SELECT doc_id FROM ds_docs WHERE status='done' AND (owner_id=? OR owner_id IS NULL)",
                    (user.id,)
                ).fetchall()
...
@ai_required
def ds_generate():
    """基于 DS 提取的知识图谱生成题目（DeepSeek 直出，无向量检索）。异步版：立即返回 task_id。"""
    import json as _json
    import re as _re
    ...
    teaching_focus_filter      = data.get('teaching_focus_filter', [])
    knowledge_type_filter      = data.get('knowledge_type_filter', [])
    cognitive_dimension_filter = data.get('cognitive_dimension_filter', [])
    ...
    batch_count = int(data.get('batch_count', 3) or 3)
    if batch_count < 2:
        batch_count = 2
    ...
    def _call_deepseek(context_str, ql_str):
        ...
        response = oa_client.chat.completions.create(
            model='deepseek-chat',
            messages=[
                {'role': 'system', 'content': '你是一位专业的教学内容设计专家，擅长根据课程知识图谱设计题库。'},
                {'role': 'user', 'content': final_prompt},
            ],
            max_tokens=8192,
            temperature=0.7,
        )
        return response.choices[0].message.content, len(context_str)
    ...
    # ── 创建任务，启动后台线程，立即返回 task_id ────────────────────────────
    task_id = 'gen_' + uuid.uuid4().hex[:8]
    _gen_tasks[task_id] = {'status': 'pending', 'content': '', 'error': '', 'stats': {}}

    def _run():
        nonlocal batch_count
        try:
            _gen_tasks[task_id]['status'] = 'running'
            import math as _math
            ...
            if not _batch_mode:
                ...
                auto_batched = True
                _batch_mode  = True
                _batch_count = 2  # 自动降级时固定分2批
                _sparse_mode = len(kps_data) > total_q * 3
            ...
            if _sparse_mode:
                ...
                for idx in range(batch_count):
                    ...
                    batch_q = sum(int(c) for c in _ql_pattern.findall(batch_ql)) if _ql_pattern.findall(batch_ql) else 0
                    q_end = q_cursor + batch_q - 1
                    kp_cnt = len(batch_samples[idx])
                    content, chars, ok, err = results.get(idx, (None, 0, False, '未收到结果'))
                    if ok:
                        merged_parts.append(f'## 第{idx + 1}批（随机采样 {kp_cnt} 个知识点，题目 {q_cursor}-{q_end}）\n\n{content}')
            ...
            batch_kps_list = []
            for idx in range(batch_count):
                q_cnt = _batch_q_count(idx)
                if q_cnt > 0 and len(kps_data) > q_cnt:
                    sampled = _sample_kps_with_relations(kps_data, q_cnt)
                else:
                    sampled = list(kps_data)
                batch_kps_list.append(sampled)
            ...
            batch_stats = []
            for idx in range(batch_count):
                kp_cnt = len(batch_kps_list[idx])
                batch_ql = _split_question_list(idx, batch_count)
                batch_q = sum(int(c) for c in _ql_pattern.findall(batch_ql)) if _ql_pattern.findall(batch_ql) else 0
                content, chars, ok, err = results.get(idx, (None, 0, False, '未收到结果'))
                if ok:
                    merged_parts.append(f'## 第{idx + 1}批（随机采样 {kp_cnt} 个知识点，共 {batch_q} 题）\n\n{content}')
                    batch_stats.append({'batch': idx + 1, 'kps': kp_cnt, 'chars': chars, 'ok': True})
                else:
                    merged_parts.append(f'## 第{idx + 1}批（随机采样 {kp_cnt} 个知识点，共 {batch_q} 题）\n\n⚠️ 本批生成失败：{err}')
                    batch_stats.append({'batch': idx + 1, 'kps': kp_cnt, 'chars': 0, 'ok': False, 'error': err})
            ...
            _gen_tasks[task_id].update({'status': 'error', 'error': str(e), 'content': ''})
...
# ─────────────────────────────────────────────────────────────────────────────
# 示例知识图谱（所有用户可见，AI 用户可软删除）
# ─────────────────────────────────────────────────────────────────────────────

_DEMO_KP_IDS = [8, 9, 10, 11, 12]  # 农产品供给曲线及其关联知识点


@rag_bp.route('/api/rag/demo-graph', methods=['GET'])
@login_required
def get_demo_graph():
    """返回示例知识图谱节点；AI 用户隐藏的节点不返回。"""
    user = get_current_user()

    # 查该用户已隐藏的节点
    from sqlalchemy import text as _text
    from app.db_models import db as _db
    hidden_ids = set()
    if user:
        rows = _db.session.execute(
            _text('SELECT kp_id FROM demo_kp_hidden WHERE user_id=:uid'),
            {'uid': user.id}
        ).fetchall()
        hidden_ids = {r[0] for r in rows}

    placeholders = ','.join('?' * len(_DEMO_KP_IDS))
    with _ds_db_conn() as conn:
        rows = conn.execute(
            f'SELECT id, kp_name, kp_content, relations_json, chapter_name '
            f'FROM ds_kps WHERE id IN ({placeholders})',
            _DEMO_KP_IDS
        ).fetchall()
    ...
    for r in rows:
        if r['id'] in hidden_ids:
            continue
        # 关联关系只保留指向其他示例节点的边
        all_rels = _json_mod.loads(r['relations_json'] or '[]')
        nodes.append({
            'id': r['id'],
            'name': r['kp_name'],
            'content': (r['kp_content'] or '')[:200],
            'chapter': r['chapter_name'],
            'relations': all_rels,
        })

    return jsonify({'nodes': nodes, 'total': len(_DEMO_KP_IDS), 'hidden': len(hidden_ids)})


@rag_bp.route('/api/rag/demo-graph/<int:kp_id>', methods=['DELETE'])
@login_required
@ai_required
def hide_demo_node(kp_id):
    """AI 用户对自己隐藏某个示例节点（软删除，其他用户不受影响）。"""
    if kp_id not in _DEMO_KP_IDS:
    ...
    try:
        _db.session.execute(
            _text('INSERT OR IGNORE INTO demo_kp_hidden (user_id, kp_id) VALUES (:uid, :kid)'),
            {'uid': user.id, 'kid': kp_id}
        )
        _db.session.commit()
    except Exception:
