...
| `language` | `String(10)` | 默认 `'zh'`, INDEX | 语言标记（`zh`/`en`/`both`） |
...
- `to_dict()` — 将模型转换为 dict，自动反序列化 JSON 字段（`options` → list，`metadata_json` → dict），时间字段转为 ISO 格式字符串。返回值直接用于 API JSON 响应。
...
**业务逻辑：**
1. 从请求 JSON 中提取字段
2. `options` 和 `metadata` 序列化为 JSON 字符串存入数据库
3. 自动设置 `created_at` 和 `updated_at` 为当前时间
4. 如果 `question_id` 重复，返回 400 错误（IntegrityError 捕获并回滚）
...
**业务逻辑：** 仅更新请求中包含的字段，未提及的字段保持不变。自动更新 `updated_at`。
...
遍历 config 中的每种题型:
    ...
    3. 取前 count 道题目
    4. 将每道题标记为 is_used=True，记录 used_date
    5. 在 exam_questions 关联表中插入记录，分配递增的 position
最终一次性 commit() 提交所有更改
```
...
4. 旧题目标记为 `is_used=False`，`used_date=None`
5. 新题目标记为 `is_used=True`，`used_date=now`
6. 关联表中删除旧记录、插入新记录，保持相同的 `position`
...
调用 `utils.export_exam_to_word()` 生成 `.docx` 文件并返回下载。
...
- 选择题（`has_options=True`）：生成带 `[A][B][C][D]` 选项的示例
- 非选择题（`has_options=False`）：生成带 `<参考答案>` 的示例

返回值：模板文件路径（存放在 `temp/question_template.docx`）

### 7.3 `export_exam_to_word(exam, filepath)`
...
**与旧版的区别：** 旧版接收 `Exam` 对象（内存数据类），选项直接是 list。新版接收 `ExamModel` 对象（ORM 模型），选项需要从 JSON 字符串反序列化：
...
| 函数 | 说明 |
|------|------|
| `create_question_in_db(db, ...)` | 直接向数据库插入题目，用于准备测试数据 |
...
| | `test_generate_marks_selected_as_used` | 组卷后标记已使用 |
| | `test_confirm_then_generate_no_overlap` | 确认后再组卷不重复 |
| | `test_replace_updates_used_flags` | 替换更新使用状态 |
...
替换操作涉及 4 个数据修改，在同一个事务中完成：

1. `old_question.is_used = False`
2. `new_question.is_used = True`
3. `DELETE FROM exam_questions WHERE exam_id=? AND question_id=old_id`
4. `INSERT INTO exam_questions (exam_id, question_id, position) VALUES (?, new_id, original_position)`

所有修改通过一次 `db.session.commit()` 提交，保证数据一致性。
