...
- **批量删除** — 多选题目后一键批量删除，自动清理试卷关联
- **题型管理** — 支持自定义题型的添加、编辑、删除，内置题型不可删除
- **自动组卷** — 根据动态题型配置，从未使用的题目中自动抽取组成试卷
- **动态组卷配置** — 组卷页题型配置行可动态增删，不再硬编码题型
- **使用追踪** — 标记已使用题目，防止跨年度重复出题；支持确认和撤销操作
- **题目替换** — 在试卷中替换同类型题目，自动更新使用状态
- **Word 导入** — 从 `.docx` 或 `.txt` 文件批量导入题目，支持自定义题型的通用解析
- **Word 导出** — 将试卷导出为格式化的 `.docx` 文件，按题型分组排列
- **动态模板生成** — 导入模板自动包含所有已注册题型（含自定义题型）的格式示例
- **双语支持** — 题目支持中文 (`zh`)、英文 (`en`) 和双语 (`both`) 三种语言标记
- **搜索过滤** — 按关键词、题型、语言多维度搜索题目
- **单页应用** — 前端为完整 SPA（四标签页），通过 REST API 与后端通信
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/questions` | 获取题目列表（支持 `keyword`/`type`/`language` 查询参数） |
| `POST` | `/api/questions` | 新增题目 |
| `GET` | `/api/questions/<id>` | 获取单个题目 |
| `PUT` | `/api/questions/<id>` | 更新题目 |
| `DELETE` | `/api/questions/<id>` | 删除题目 |
| `POST` | `/api/questions/import` | 从文件导入题目（multipart/form-data） |
...
| `GET` | `/api/exams/<id>/export` | 导出试卷为 Word 文档 |
