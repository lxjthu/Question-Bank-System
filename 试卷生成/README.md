...
- **Word 导入** — 从 `.docx` 或 `.txt` 文件批量导入，支持双语内容、元数据解析、多段落题目（材料分析）；导入时自动查重，重复题目跳过并在结果中计数
- **Word 导出** — 将试卷导出为格式化的 `.docx` 文件，支持中文/英文/中英对照三种模式，可选含答案或仅题目；层级题型（`简答>材料分析`）在导出中只显示子类型名（`材料分析`）
- **动态模板生成** — 导入模板自动包含所有已注册题型的格式示例，含双语和元数据格式说明
...
├── word_to_csv_converter.py        # Word 文档解析器（docx → 题目列表）
├── migrate_add_subject_column.py   # 迁移脚本：添加 subject 字段
...
**格式要点：**
- 题型标识符支持层级格式 `简答>计算`，导出 Word 时只显示子类型（`计算`）
- 英文选项使用 `[A_en]`、`[B_en]` 等标记
- `<解析>` 区段内可包含元数据：`科目:`、`知识点:`、`标签:`、`英文题目:`、`难度:`
- 材料分析等多段落题目内容完整支持，不限段落数
- 导入时自动查重（按题目内容去重），重复题目跳过并在返回结果中给出计数
